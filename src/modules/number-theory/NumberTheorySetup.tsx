...
interface Props {
  onStart: (s: NumberTheorySettings) => void;
...
// Multi-select chip picker over the Number-Theory skill set. Mirrors the
// pattern in ArithmeticSetup / ShapesSetup: refuses to clear the last chip
// so the generator always has something to draw from.
interface SkillChipPickerProps {
  selected: NumberTheorySkill[];
  onChange: (next: NumberTheorySkill[]) => void;
}

function SkillChipPicker({ selected, onChange }: SkillChipPickerProps) {
  const orderIndex = new Map<NumberTheorySkill, number>(
    NUMBER_THEORY_SKILL_OPTIONS.map((s, i) => [s, i])
  );
  const isSelected = (s: NumberTheorySkill) => selected.includes(s);
  const toggle = (s: NumberTheorySkill) => {
    if (isSelected(s)) {
  ...
  return (
    <div className="grid grid-cols-2 gap-1 md:gap-2">
      {NUMBER_THEORY_SKILL_OPTIONS.map(s => (
        <button
          key={s}
          type="button"
          onClick={() => toggle(s)}
...
export function NumberTheorySetup({
  onStart,
  currentUser,
  onUserChange,
  onNewUser,
  ...
  useEffect(() => {
    setIsLoaded(false);
    const s = getSavedNumberTheorySettings(currentUser?.id);
    setSkills(s.skills);
    setDifficulty(s.difficulty);
    setGameMode(s.gameMode);
    setQuestionCount(s.questionCount);
    setTimeLimit(s.timeLimit);
    setPrintConfig(getSavedNumberTheoryPrintConfig(currentUser?.id));
    setIsLoaded(true);
  }, [currentUser?.id]);
  ...
  const handlePrintDownload = (pages: number, perPage: number, name: string) => {
    ...
    const pagesArr = Array.from({ length: pages }, () =>
      generateNumberTheoryQuestions(settings, perPage)
    );
    const subtitle = `${perPage} Questions per page — ${buildNumberTheorySummary(skills, difficulty)}`;
    const doc = generateNumberTheoryPdf({
      pages: pagesArr,
      title: 'Maths Challenge — Number Theory',
      subtitle,
      studentName: name || undefined,
      includeAnswerKey: true,
    });
    doc.save('maths-number-theory.pdf');
    const next = { pageCount: pages, questionsPerPage: perPage };
    setPrintConfig(next);
    saveNumberTheoryPrintConfig(next, currentUser?.id);
  };
  ...
  return (
    <div className="min-h-screen bg-background p-7">
      ...
      <PrintWorksheetModal
        ...
        summary={buildNumberTheorySummary(skills, difficulty)}
