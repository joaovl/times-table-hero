...
// Difficulty tokens are canonical values; buttons display the translation.
const DIFFICULTY_KEY: Record<string, MessageKey> = {
  easy: 'shapes.setup.difficultyEasy',
  medium: 'shapes.setup.difficultyMedium',
  hard: 'shapes.setup.difficultyHard',
};
import type {
  ShapeDifficulty,
  ShapeSettings,
...
import {
  getSavedShapePrintConfig,
  getSavedShapeSettings,
  saveShapePrintConfig,
  saveShapeSettings,
} from './storage';
import {
  PRINT_PAGE_OPTIONS,
...
// Always non-empty chip picker — refuses to clear the last chip.
interface SkillChipPickerProps {
  selected: ShapeSkill[];
  onChange: (next: ShapeSkill[]) => void;
}

function SkillChipPicker({ selected, onChange }: SkillChipPickerProps) {
  const orderIndex = new Map<ShapeSkill, number>(
    SHAPE_SKILL_OPTIONS.map((s, i) => [s, i])
  );
  const isSelected = (s: ShapeSkill) => selected.includes(s);
  const toggle = (s: ShapeSkill) => {
    if (isSelected(s)) {
  ...
  return (
    <div className="grid grid-cols-2 gap-1 md:gap-2">
      {SHAPE_SKILL_OPTIONS.map(s => (
        ...
        >
          <span className="text-[13px] md:text-[15px]">{shapeSkillLabel(s)}</span>
        </button>
      ))}
    </div>
  );
...
export function ShapesSetup({
  ...
  const { t } = useT();
  const [skills, setSkills] = useState<ShapeSkill[]>(['name-2d']);
  const [units, setUnits] = useState<ShapeUnits>('cm');
  const [difficulty, setDifficulty] = useState<ShapeDifficulty>('easy');
  const [gameMode, setGameMode] = useState<'questions' | 'time'>('questions');
  const [questionCount, setQuestionCount] = useState(10);
  const [timeLimit, setTimeLimit] = useState(180);
  const [isLoaded, setIsLoaded] = useState(false);
  const [printOpen, setPrintOpen] = useState(false);
  const [printConfig, setPrintConfig] = useState({ pageCount: 1, questionsPerPage: 12 });

  useEffect(() => {
    setIsLoaded(false);
    const s = getSavedShapeSettings(currentUser?.id);
    setSkills(s.skills);
    setUnits(s.units);
    ...
    setPrintConfig(getSavedShapePrintConfig(currentUser?.id));
    setIsLoaded(true);
  }, [currentUser?.id]);
  ...
  useEffect(() => {
    if (!isLoaded) return;
    saveShapeSettings(
      { skills, units, difficulty, gameMode, questionCount, timeLimit },
      currentUser?.id
    );
  }, [isLoaded, skills, units, difficulty, gameMode, questionCount, timeLimit, currentUser?.id]);
  ...
  const start = () =>
    onStart({ skills, units, difficulty, gameMode, questionCount, timeLimit });
  ...
  const handlePrintDownload = (pages: number, perPage: number, name: string) => {
    ...
    const pagesArr = Array.from({ length: pages }, () => generateShapeQuestions(settings, perPage));
    const subtitle = `${perPage} Questions per page — ${buildShapesSummary(skills, units, difficulty)}`;
    const doc = generateShapesPdf({
      pages: pagesArr,
      title: 'Maths Challenge — Shapes',
      subtitle,
      studentName: name || undefined,
      includeAnswerKey: true,
    });
    doc.save('maths-shapes.pdf');
    const next = { pageCount: pages, questionsPerPage: perPage };
    setPrintConfig(next);
    saveShapePrintConfig(next, currentUser?.id);
  };
    ...
    <div className="min-h-screen bg-background p-7">
      <div className="mx-auto max-w-[600px]">
        ...
        <h1 className="text-[22px] md:text-[36px] font-bold text-primary text-center mb-1 md:mb-2">
          {t('shapes.setup.title')}
        </h1>
        <p className="text-center text-[12px] md:text-[17px] text-muted-foreground mb-3">
          {currentUser ? t('shapes.setup.hiName', { name: currentUser.name }) : ''}{t('shapes.setup.subtitle')}
        </p>
        ...
        <Card className="mb-2 md:mb-4 p-3 md:p-5 shadow-card">
          <h2 className="mb-2 md:mb-3 text-[14px] md:text-[20px] font-semibold text-foreground">{t('shapes.setup.skills')}</h2>
          <SkillChipPicker selected={skills} onChange={setSkills} />
        </Card>

        <Card className="mb-2 md:mb-4 p-3 md:p-5 shadow-card">
          <h2 className="mb-2 md:mb-3 text-[14px] md:text-[20px] font-semibold text-foreground">{t('shapes.setup.units')}</h2>
          <div className="grid grid-cols-4 gap-2">
            {SHAPE_UNIT_OPTIONS.map(u => (
              <button key={u} onClick={() => setUnits(u)} className={buttonClass(units === u)}>
                <span className="text-[13px] md:text-[16px]">{u}</span>
              </button>
            ))}
          </div>
        </Card>
        ...
        <Card className="mb-2 md:mb-4 p-3 md:p-5 shadow-card">
          <h2 className="mb-2 md:mb-3 text-[14px] md:text-[20px] font-semibold text-foreground">{t('shapes.setup.difficulty')}</h2>
          <div className="grid grid-cols-3 gap-2">
            {SHAPE_DIFFICULTY_OPTIONS.map(d => (
        ...
        <Card className="mb-2 md:mb-4 p-3 md:p-5 shadow-card">
          <h2 className="mb-2 md:mb-3 text-[14px] md:text-[20px] font-semibold text-foreground">{t('shapes.setup.gameMode')}</h2>
          <div className="flex gap-2 mb-3">
            <button onClick={() => setGameMode('questions')} className={cn('flex-1', buttonClass(gameMode === 'questions'))}>
              <span className="text-[13px] md:text-[16px]">{t('shapes.setup.questions')}</span>
            </button>
            <button onClick={() => setGameMode('time')} className={cn('flex-1', buttonClass(gameMode === 'time'))}>
              <span className="text-[13px] md:text-[16px]">{t('shapes.setup.timed')}</span>
            </button>
          </div>
          {gameMode === 'questions' ? (
      ...
      <PrintWorksheetModal
        ...
        summary={buildShapesSummary(skills, units, difficulty)}
