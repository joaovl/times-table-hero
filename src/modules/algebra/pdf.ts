...
import type { AlgebraQuestion } from './logic';
import { answerText, questionPromptText } from './logic';

export interface AlgebraPdfOptions {
  pages: AlgebraQuestion[][];
  title: string;
  subtitle: string;
  studentName?: string;
...
function drawHeader(doc: jsPDF, title: string, subtitle: string, studentName?: string) {
  const left = MARGIN;
  const top = MARGIN;
...
export function answerKeyText(q: AlgebraQuestion, num: number): string {
...
export function generateAlgebraPdf(opts: AlgebraPdfOptions): jsPDF {
