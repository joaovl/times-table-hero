...
import { generateNumberSensePdf } from './pdf';
import {
  getSavedNumberSenseSettings,
  saveNumberSenseSettings,
  getSavedNumberSensePrintConfig,
  saveNumberSensePrintConfig,
} from './storage';
import {
  PRINT_PAGE_OPTIONS,
...
interface Props {
  onStart: (s: NumberSenseSettings) => void;
...
const DIFFICULTY_HINT_KEYS: [MessageKey, MessageKey, MessageKey] = [
  'numberSense.setup.hintEasy',
  'numberSense.setup.hintMedium',
  'numberSense.setup.hintHard',
];

const DIFFICULTY_LABEL_KEY: Record<Difficulty, MessageKey> = {
  easy: 'numberSense.setup.difficultyEasy',
  medium: 'numberSense.setup.difficultyMedium',
  hard: 'numberSense.setup.difficultyHard',
};
...
export function NumberSenseSetup({
  onStart,
  currentUser,
  onUserChange,
  onNewUser,
  ...
  const { t } = useT();
  const [skills, setSkills] = useState<NumberSenseSkill[]>(['place-value-3d']);
  ...
  useEffect(() => {
    setIsLoaded(false);
    const s = getSavedNumberSenseSettings(currentUser?.id);
    setSkills(s.skills);
    setDifficulty(s.difficulty);
    setGameMode(s.gameMode);
    setQuestionCount(s.questionCount);
    setTimeLimit(s.timeLimit);
    setPrintConfig(getSavedNumberSensePrintConfig(currentUser?.id));
    setIsLoaded(true);
  }, [currentUser?.id]);
  ...
  const toggleSkill = (s: NumberSenseSkill) => {
    setSkills(prev => {
      if (prev.includes(s)) {
        if (prev.length === 1) return prev;
        return prev.filter(x => x !== s);
      }
      // Preserve canonical order from ALL_SKILLS so the chip set always
      // reads top-to-bottom in the order the parent saw them.
  ...
  const handlePrintDownload = (pages: number, perPage: number, name: string) => {
    ...
    const pagesArr = Array.from({ length: pages }, () =>
      generateNumberSenseQuestions(settings, perPage)
    );
    const subtitle = buildNumberSenseSummary(skills, difficulty);
    const doc = generateNumberSensePdf({
      pages: pagesArr,
      title: 'Maths Challenge — Number Sense',
      subtitle: `${perPage} Questions per page — ${subtitle}`,
      studentName: name || undefined,
      includeAnswerKey: true,
    });
    doc.save('maths-number-sense.pdf');
    const next = { pageCount: pages, questionsPerPage: perPage };
    setPrintConfig(next);
    saveNumberSensePrintConfig(next, currentUser?.id);
  };
    ...
    <div className="min-h-screen bg-background p-7">
      <div className="mx-auto max-w-[600px]">
        ...
        <h1 className="text-[22px] md:text-[36px] font-bold text-primary text-center mb-1 md:mb-2">
          {t('numberSense.setup.title')}
        </h1>
        <p className="text-center text-[12px] md:text-[17px] text-muted-foreground mb-3">
          {currentUser ? t('numberSense.setup.hiName', { name: currentUser.name }) : ''}{t('numberSense.setup.subtitle')}
        </p>
        ...
        <Card className="mb-2 md:mb-4 p-3 md:p-5 shadow-card">
          <h2 className="mb-2 md:mb-3 text-[14px] md:text-[20px] font-semibold text-foreground">
            {t('numberSense.setup.difficulty')}
          </h2>
        ...
        <Card className="mb-2 md:mb-4 p-3 md:p-5 shadow-card">
          <h2 className="mb-2 md:mb-3 text-[14px] md:text-[20px] font-semibold text-foreground">{t('numberSense.setup.gameMode')}</h2>
          <div className="flex gap-2 mb-3">
            <button
              onClick={() => setGameMode('questions')}
              className={cn('flex-1', buttonClass(gameMode === 'questions'))}
            >
              <span className="text-[13px] md:text-[16px]">{t('numberSense.setup.questions')}</span>
            </button>
            <button
              onClick={() => setGameMode('time')}
              className={cn('flex-1', buttonClass(gameMode === 'time'))}
            >
              <span className="text-[13px] md:text-[16px]">{t('numberSense.setup.timed')}</span>
            </button>
          </div>
          {gameMode === 'questions' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
          <Button variant="outline" onClick={() => setPrintOpen(true)} className="py-3 font-bold">
            {t('numberSense.setup.printWorksheet')}
          </Button>
          <Button
            onClick={start}
      ...
      <PrintWorksheetModal
        ...
        summary={buildNumberSenseSummary(skills, difficulty)}
