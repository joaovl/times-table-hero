...
import {
  ...
  saveStatsSettings,
  getSavedStatsPrintConfig,
  saveStatsPrintConfig,
} from './storage';
import { PRINT_PAGE_OPTIONS, PRINT_PER_PAGE_OPTIONS, buildStatsSummary } from './printConfig';

interface Props {
  onStart: (s: StatsSettings) => void;
...
export function StatisticsSetup({
  onStart,
  currentUser,
  onUserChange,
  onNewUser,
  ...
  useEffect(() => {
    setIsLoaded(false);
    const s = getSavedStatsSettings(currentUser?.id);
    setSkills(s.skills);
    setDifficulty(s.difficulty);
    setGameMode(s.gameMode);
    setQuestionCount(s.questionCount);
    setTimeLimit(s.timeLimit);
    setPrintConfig(getSavedStatsPrintConfig(currentUser?.id));
    setIsLoaded(true);
  }, [currentUser?.id]);

  useEffect(() => {
    if (!isLoaded) return;
    saveStatsSettings({ skills, difficulty, gameMode, questionCount, timeLimit }, currentUser?.id);
  ...
  const toggleSkill = (s: StatsSkill) => {
    setSkills(prev => {
      if (prev.includes(s)) {
  ...
  const handlePrintDownload = (pages: number, perPage: number, name: string) => {
    ...
    const pagesArr = Array.from({ length: pages }, () => generateStatsQuestions(settings, perPage));
    const subtitle = buildStatsSummary(skills, difficulty);
    const doc = generateStatsPdf({
      pages: pagesArr,
    ...
    doc.save('maths-statistics.pdf');
    const next = { pageCount: pages, questionsPerPage: perPage };
    setPrintConfig(next);
    saveStatsPrintConfig(next, currentUser?.id);
  };
  ...
  return (
    <div className="min-h-screen bg-background p-7">
      ...
      <PrintWorksheetModal
        ...
        summary={buildStatsSummary(skills, difficulty)}
